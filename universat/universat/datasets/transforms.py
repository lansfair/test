...
def _load_array(path: str) -> np.ndarray:
    ...
    Supports ``.npy`` files and common image formats (``.jpg``, ``.jpeg``,
    ``.png``, ``.bmp``, ``.tif``, ``.tiff``). Images are returned as
    ``(C, H, W)`` float32 arrays.
    """
    ...
    raise ValueError(
        ...
        f"UniverSat transforms currently support .npy and image files."
    )
