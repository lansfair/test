...
export CONDA_ENV_NAME="${1:-"openmm-10m"}"
...
rm -rf "$PLOT_DIR" > /dev/null

PLOT_INFER_DIR="$PLOT_DIR/infer"
$PYINTERPRETER "$PWD/tools/test.py" "$SRCDIR/configs/$CONFIG_NAME.py" "$SRCDIR/checkpoints/$(basename $CONFIG_NAME).pth" --work-dir "$WORK_DIR" --out "$PLOT_INFER_DIR"

DATADIR="/mnt/ht2-nas2/EO_test/zyf/data/geobench2tif/m-SA-crop-type/test"
$PYINTERPRETER "$SRCDIR/vis_compare_all.py" "$DATADIR" "$PLOT_INFER_DIR" "$PLOT_DIR/compare"