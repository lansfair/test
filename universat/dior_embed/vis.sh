...
export MM_WORK_DIR_HOME="/mnt/ht2-nas2/EO_test"
...
# 启动配置名称，与 extract_embeddings 输出的目录名一致。
export CONFIG_NAME="${3:-"self-olmoearth-base-10m-bak_1xb8-50e_dior-rgb-embed-linear"}"

# 可视化样本数上限，0 表示全部。
export MAX_SAMPLES="${4:-10}"
...
EMBED_INPUT_DIR="${MM_ARCHIVE_EMBED_HOME}/${CONFIG_NAME}"
PLOT_OUTPUT_DIR="${MM_ARCHIVE_EMBED_HOME}/${CONFIG_NAME}-visual"
mkdir -p "$PLOT_OUTPUT_DIR"

echo "input:  $EMBED_INPUT_DIR"
echo "output: $PLOT_OUTPUT_DIR"
echo "max samples: $MAX_SAMPLES"

$PYINTERPRETER "$SRCDIR/tools/visualize_embed_and_raw.py" \
    --input-root "$EMBED_INPUT_DIR" \
    --output-root "$PLOT_OUTPUT_DIR" \
