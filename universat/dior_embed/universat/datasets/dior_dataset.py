...
class UniverSatDIORDataset(UniverSatSegDataset):
    ...
    def get_data_info(self, idx: int) -> dict:
        ...
        # [modified 20260716] 注释上一段，采用下一段代码
        # 由于 DIOR 数据集路径中没有 Annotations/test ，需要在 xml 缺失时回退到全零 mask ，否则 test split 会再次触发gt_sem_seg 不存在的错误 
        if self.bbox_to_mask:
            if xml_path is not None and os.path.exists(xml_path):
                ...
                import warnings
                sample_id = info.get("sample_id", idx)
                warnings.warn(
                    f"DIOR annotation XML not found for sample "
                    f"{sample_id}: {xml_path}. "
                    f"Falling back to an all-zero semantic mask."
                )
                info["gt_seg_map"] = np.zeros((height, width), dtype=np.uint8)
