...
class Dynamic_MLP_OFA_variable(nn.Module):
    ...
    def __init__(self, wv_planes, inter_dim=128, kernel_size=3, embed_dim=1024, option='language'):
        ...
        if self.option == 'spectrum':
            ...
            env_embed_path = os.environ.get('COPERNICUS_FM_VAR_EMBED', '')
            if not os.path.exists(llm_embed_path) and env_embed_path and os.path.exists(env_embed_path):
                llm_embed_path = env_embed_path
            if not os.path.exists(llm_embed_path):
                raise FileNotFoundError(
                    "Dynamic_MLP_OFA_variable(option='language') requires "
                    "var_embed_llama3.2_1B.pt, but it was not found. This OpenMMLab "
                    "package intentionally does not auto-download it, because the old "
                    "HuggingFace URL can return HTTP 404 on training clusters. For "
                    "PASTIS Sentinel-2 spectral segmentation, set backbone input_mode='spectral' "
                    "and var_option='spectrum' instead. If you really need variable-mode, "
                    "put var_embed_llama3.2_1B.pt under cfm_mmseg/copernicus_fm/ or set "
                    "COPERNICUS_FM_VAR_EMBED=/absolute/path/var_embed_llama3.2_1B.pt."
                )
