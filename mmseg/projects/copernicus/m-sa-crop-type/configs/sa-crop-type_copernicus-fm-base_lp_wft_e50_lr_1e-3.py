...
_base_ = [
    ...
    './dataset/sa-crop-type.py'
]

PATCH_AREA = (16 * 10 / 1000)**2

S2_BAND_WAVELENGTHS = (
    440,  # 01 - Coastal aerosol
    490,  # 02 - Blue
    560,  # 03 - Green
    665,  # 04 - Red
...
BACKBONE_CHECKPOINT_PATH = os.path.join(os.environ.get('MM_ARCHIVE_CKPT_HOME'), "CopernicusFM_ViT_base_varlang_e100.pth")
...
custom_imports = dict(imports=['projects.copernicus.m-sa-crop-type.copernicus'], allow_failed_imports=False)

custom_hooks = [
    dict(
        type='OlmoEarthFTStrategyHook',
        total_epoch=TRAIN_EPOCH,
...
model = dict(
    ...
    backbone=dict(
        ...
        init_cfg=dict(
            type='Pretrained',
            checkpoint=BACKBONE_CHECKPOINT_PATH,
        ),
