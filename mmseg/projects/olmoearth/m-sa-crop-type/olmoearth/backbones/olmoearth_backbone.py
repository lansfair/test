...
class OlmoEarthBackbone(BaseModule):
    ...
    def _freeze_unused_pretrain_parameters(self) -> None:
        """Freeze OLMoEarth branches unused by downstream dense prediction.

        The released checkpoint may include decoder, target_encoder, and many
        modality-specific encoder branches. Downstream OpenMMLab forward only
        calls the online encoder for ``self.modality``; leaving all other
        parameters trainable makes DDP fail with unused-parameter errors.
        """
