...
class RGBToOlmoEarthS2(BaseTransform):
    ...
    def __init__(
        ...
        std_multiplier: float = 2.0,
        keep_raw_input: bool = False, #张浩晨修改
    ) -> None:
        ...
        self.std_multiplier = std_multiplier
        self.keep_raw_input = keep_raw_input    #张浩晨修改
    ...
    def transform(self, results: dict[str, Any]) -> dict[str, Any]:
        ...
        if self.keep_raw_input: #张浩晨修改
            results['olmoearth_raw_img'] = np.ascontiguousarray(image)
            results['olmoearth_raw_band_names'] = [
                c for c in self.rgb_channel_order
            ]
