...
@TRANSFORMS.register_module()
class LoadPotsdamOlmoEarthEmbedding(BaseTransform):
    """Load precomputed dense OLMoEarth embeddings for linear probing.
    ...
    def transform(self, results: dict[str, Any]) -> dict[str, Any]:
        ...
        ori_img = _read_geotiff(os.path.dirname(results['embedding_path'])+'/raw_input.tif').astype(self.dtype, copy=False)  #张浩晨修改
        results["img"] = np.moveaxis(embedding, 0, -1)
        results["gt_seg_map"] = label
        results["ori_shape"] = label.shape
        results["img_shape"] = results["img"].shape[:2]
        results["olmoearth_raw_img"] = ori_img  #张浩晨修改
