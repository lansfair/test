...
class SSLFeatureDistillationMetaArch(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        if teacher_args.arch == "vit_7b": teacher_args.qkv_bias, teacher_args.ffn_layer, teacher_args.ffn_ratio, teacher_args.proj_bias, teacher_args.ffn_bias = False, "swiglu64", 3, True, True  # modified by zhoujiwen
        ...
        teacher_args.untie_cls_and_patch_norms, teacher_args.untie_global_and_local_cls_norm = False, teacher_args.arch == "vit_7b"  # modified by zhoujiwen
        teacher_backbone, teacher_dim = build_model(teacher_args, only_teacher=True, img_size=cfg.crops.global_crops_size, device="meta")
        student_dim = embed_dim * 8 if "swin" in cfg.student.arch else embed_dim  # modified by zhoujiwen
        student_feature_proj = nn.Sequential(nn.Linear(student_dim, teacher_dim, bias=False), nn.LayerNorm(teacher_dim)) if student_dim != teacher_dim else nn.Identity()  # modified by zhoujiwen
        ema_feature_proj = copy.deepcopy(student_feature_proj)  # modified by zhoujiwen
        self.student, self.model_ema, self.teacher = nn.ModuleDict({"backbone": student_backbone, "feature_proj": student_feature_proj}), nn.ModuleDict({"backbone": ema_backbone, "feature_proj": ema_feature_proj}), nn.ModuleDict({"backbone": teacher_backbone})  # modified by zhoujiwen
    ...
    def init_weights(self) -> None:
        ...
        self.student.feature_proj.apply(lambda m: m.reset_parameters() if hasattr(m, "reset_parameters") else None)  # modified by zhoujiwen
    ...
    def _project_student(self, x):
        return self.student.feature_proj(x)  # modified by zhoujiwen
    ...
    def _forward_loss(self, data):
        ...
        dino_global_loss = self._feature_loss(self._project_student(s_global["x_norm_clstoken"]), t_global["x_norm_clstoken"])  # modified by zhoujiwen
        dino_local_loss = self._feature_loss(self._project_student(s_local["x_norm_clstoken"]), t_cls_img.repeat(n_local, 1)) if n_local else dino_global_loss.new_zeros(())  # modified by zhoujiwen
        dino_loss = self.cfg.dino.loss_weight * 0.5 * (dino_global_loss + dino_local_loss)
        s_patch = self._project_student(self._upsample_student_patches(s_global["x_norm_patchtokens"], t_global["x_norm_patchtokens"].shape[1]))  # modified by zhoujiwen
