...
def get_args_parser(add_help: bool = True):
    ...
    parser.add_argument("--config-file", default="/mnt/qh2-nas3/00-model/00-wrs/dinov3_workspace/code_revised/dinov3-swin/dinov3/configs/train/eurosat_pretrain_swin_ibot.yaml",
...
def build_schedulers(cfg):
    ...
    logger.info("not Using schedules v2")
    OFFICIAL_EPOCH_LENGTH = cfg.train.OFFICIAL_EPOCH_LENGTH
    print(f'115 base lr:{cfg.optim["lr"]} warmup_epoch{cfg.optim["warmup_epochs"]}')
    lr = dict(
...
def do_train(cfg, model, resume=False):
    ...
    model.init_weights()
    start_iter = 0
    logger.info(f"ckpt_dir: {str(ckpt_dir)} resume: {resume} ")
 ...
 #注释       if cfg.gram.use_loss and model.gram_it_load_ema_teacher == it:
 ...
 #           num_gram_updates += 1
        ...
        if not feature_distill_enabled and (iteration + 1) % OFFICIAL_EPOCH_LENGTH == 0:
            ...
            if val_loss_for_csv < best_val_loss:
                ...
                bad_val_count += 1
            if cfg.feature_distill.early_stopping_patience >= 0 and bad_val_count >= cfg.feature_distill.early_stopping_patience:
...
def main(argv=None):
    ...
    if args.multi_distillation:
        ...
        logger.info("%s", cfg)
        setup_logging(
            output=os.path.join(os.path.abspath(cfg.train.output_dir), "nan_logs"),
            name="nan_logger",
        )
    meta_arch = {
    ...
    # )
    model._apply(
        lambda t: torch.zeros_like(t, device="cuda"),
        recurse=True,
    )
